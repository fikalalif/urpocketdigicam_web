    ...
    <div class="container mx-auto p-4">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white shadow-md rounded-lg">
                <thead>
                    <tr>
                        <th class="py-2 px-4 bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase">Nama
                            Produk</th>
                        <th class="py-2 px-4 bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase">Harga
                        </th>
                        <th class="py-2 px-4 bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase">
                            Visibilitas Produk</th>
                        <th class="py-2 px-4 bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase">Sinkron
                            Ke Hub
                        </th>
                        <th class="py-2 px-4 bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase">Aksi
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($products as $product)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 border-b">
                                @if ($product->hub_product_id)
                                    <div x-data="{
                                        ...
                                        csrf-token"]').getAttribute('content'); fetch(url, { method: 'PUT' , headers:
                                        { 'Content-Type' : 'application/json' , 'Accept' : 'application/json' , 'X-CSRF-TOKEN' :
                                        csrfToken, }, body: JSON.stringify({ is_on: this.isOn }) }) .then(response=> {
                                        if (!response.ok) {
                                        ...
                                        <button type="button" x-on:click="isOn = !isOn; toggleVisibility()" :aria-checked="isOn"
                            ...
                            <td class="py-3 px-4 border-b">
                                ...
                                @if (!$product->hub_product_id)
                                    <button x-data="{ productId: '{{ $product->id }}' }" x-on:click="
                            ...
                            <td>
                                <div class="flex items-center space-x-2">
                                    ...
                                    <a href="{{ route('products.edit', $product->id) }}"
                                        class="bg-green-500 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded text-sm">
                                        Edit
                                    </a>
                                    ...
                                    <form action="{{ route('products.destroy', $product->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded text-sm">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
