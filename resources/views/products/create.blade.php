<x-layouts.app :title="'Tambah Produk'">
    <div class="container mx-auto px-4 py-8 animate-fade-in">
        <h1 class="text-4xl font-extrabold mb-8 text-foreground text-shadow">Tambah Produk Baru</h1>
        ...
        <div class="bg-card text-card-foreground shadow-2xl rounded-xl p-8 border border-border card-gradient">
            <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="flex items-center gap-4 pt-4">
                    ...
                    <a href="{{ route('products.index') }}" class="inline-flex items-center justify-center rounded-full text-base font-medium transition-colors h-12 px-6 py-3 bg-muted text-muted-foreground hover:bg-muted/80 shadow-md">
                        Batal
                    </a>
                </div>
            </form>
